...
class Speech2Text:
    ...
    def __call__(
        ...
        if self.enh_s2t_task:
            ...
            if len(enc) == 1:
                ...
                # branch
                # c. Passed the encoder result and the beam search
                results = self._decode_single_PIT_sample(enc)
                assert check_return_type(results[0])
    ...
    def _decode_single_PIT_sample(self, encs: torch.Tensor):

        # split enc into 2 parts
        assert encs.shape[0] == 2, encs.shape
        enc1, enc2 = torch.split(encs, 1, dim=0)
        enc1 = enc1.squeeze(0)
        enc2 = enc2.squeeze(0)
        ...
        assert not self.beam_search_transducer

        # 1. decode enc1 and enc2
        pit_results = []
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            if enh_s2t_task:
                ...
                assert nbest == 1,"PIT ASR only supports nbest=1"
                if len(results) == 1:
                    # Normal ASR
                    for n, (text, token, token_int, hyp) in zip(
                    ...
                    text1, token1, token_int1, hyp1 = results[0][0]
                    text2, token2, token_int2, hyp2 = results[1][0]

                    # Create a directory: outdir/{n}best_recog
                    ibest_writer = writer[f"1best_recog"]

                    # Write the result to each file
                    ibest_writer["token"][key] = " ".join(token1) + " $ " + " ".join(token2)
                    ibest_writer["token_int"][key] = " ".join(map(str, token_int1)) + " -1 " + " ".join(map(str,token_int2)) # -1 is the separator
                    ibest_writer["score"][key] = str(hyp1.score + hyp2.score)

                    if text1 is not None:
                        ibest_writer["text"][key] = text1 + " $ " + text2
